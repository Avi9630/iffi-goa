...
@section('content')
        ...
        <div class="tab-content" id="myTabContent">
            ...
            @foreach ($dates as $date)
                ...
                <div class="tab-pane fade" id="{{ $formattedId }}" role="tabpanel"
                    ...
                    @foreach ($topics->where('master_date_id', $date->id) as $topic)
                        @include("master-class.{$formattedId}", ['topic' => $topic])
                        {{-- @include('partials.masterclass-card.{$formattedId}', ['topic' => $topic]) --}}
                        {{-- @include('partials.masterclass-card', ['topic' => $topic]) --}}
                    @endforeach
                ...
                <div class="modal-body">
                    <ul class="list-icon">
                        <li>
                            ...
                            Moderator :- <span id="modalModerator"></span>
                        </li>
                        <li>
    ...
    <script>
        const modalData = @json($modalData);
        ...
        document.querySelectorAll('.title-tab').forEach(tab => {
            tab.addEventListener('click', function() {
                ...
                if (!data) {
                    console.warn('No data found for', dateId, sessionIndex);
                    return;
                }
